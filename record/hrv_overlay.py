"""
This script is used to generate and save visualizations of Heart Rate Variability (HRV) metrics.

It reads HRV data from an `hrv_values.csv` file, which is typically generated by
the `hrv_calc.py` script. The script then groups participants and plots their
RMSSD (Root Mean Square of Successive Differences), SDNN (Standard Deviation of NN
intervals), and pNN50 (Percentage of successive RR intervals that differ by more
than 50 ms) values across different recording segments.

The generated graphs are saved as PNG image files into a directory named `group_graphs`.
This directory is further organized into subdirectories for each group of participants
(e.g., `group_1`, `group_2`, etc.), with each subdirectory containing the plots for
that specific group.

This script is useful for visual comparison of HRV trends across different participants
and recording segments, aiding in the interpretation of physiological data.
"""
import pandas as pd
